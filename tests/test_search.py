...
def test_search_finds_note():
    """Tests that FTS can find content in a note."""
    notes_manager.create_note("Esta es una nota sobre Python y desarrollo.")
...
def test_search_finds_kanban_card():
    """Tests that FTS can find content in a Kanban card's title and description."""
    kanban_manager.create_default_columns()
    kanban_manager.create_card(1, "Refactorizar el módulo de UI", "Se necesita optimizar el rendimiento.")
...
def test_search_no_results():
    ...
    notes_manager.create_note("Nota de prueba.")
...
def test_search_prefix():
    ...
    kanban_manager.create_default_columns()
    kanban_manager.create_card(1, "Implementar la autenticación", "Usar tokens JWT.")
