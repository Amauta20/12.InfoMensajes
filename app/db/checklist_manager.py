...
def update_checklist_item(item_id, text=None, is_checked=None, due_at=None, is_notified=None, pre_notified_at=None):
    ...
    if due_at is not None:
        cursor.execute("UPDATE checklist_items SET due_at = ? WHERE id = ?", (due_at, item_id))
    ...
    if pre_notified_at is not None:
        cursor.execute("UPDATE checklist_items SET pre_notified_at = ? WHERE id = ?", (pre_notified_at, item_id))
    conn.commit()
    conn.close()
