...
def create_default_columns(db_path):
...
def create_card(db_path, column_id, title, description=None, assignee=None, due_date=None, created_at=None, started_at=None, finished_at=None, start_date=None, end_date=None):
...
def get_cards_by_column(db_path, column_id):
...
def move_card(db_path, card_id, new_column_id):
...
def delete_card(db_path, card_id):
...
def update_card(db_path, card_id, new_title, new_description=None, new_assignee=None, new_due_date=None, new_start_date=None, new_end_date=None):
...
def get_card_details(db_path, card_id):
...
def generate_kanban_report(db_path):
    conn = get_db_connection(db_path)
    cursor = conn.cursor()
...
def get_cards_due_between(db_path, start_date, end_date):
...
def get_all_kanban_cards_for_gantt(db_path):
