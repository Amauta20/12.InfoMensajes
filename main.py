...
def test_vault():
    ...
    # Save secret
    vault.save_secret(secret_id, plaintext, correct_passphrase)
    print(f"Secret '{secret_id}' saved.")

    # Retrieve with correct passphrase
    retrieved_secret = vault.get_secret(secret_id, correct_passphrase)
    ...
    retrieved_secret_wrong = vault.get_secret(secret_id, incorrect_passphrase)
